...
class CollectFSM(Node):
    """
    CollectFSM orchestrates the egg collection process.
    """
    def __init__(self):
        super().__init__("collect_fsm")

        cb_group = ReentrantCallbackGroup()
        self.egg_id_client = self.create_client(IdentifyEgg, "egg/identify", callback_group=cb_group)
        self.handle_egg_client = PatchRclpyIssue1123(self, GenericTask, "exec_handle_fsm", callback_group=cb_group)

        self.action_server = ActionServer(
            self, GenericTask, "exec_collect_fsm", self.action_server_callback,
    ...
    def action_server_callback(self, goal_handle):
        ...
        self.cancel_flag = False
        try:
            self.state = State.IDENTIFYING_EGG
            self.run_state_machine()
            goal_handle.succeed()
        except Exception as e:
    ...
    def run_state_machine(self):
        while self.state != State.IDLE:
            ...
            if self.state == State.IDENTIFYING_EGG:
                egg_type = asyncio.run(self.identify_egg())
                if egg_type is not None:
                    self.egg_type = egg_type
                    self.state = State.HANDLING_EGG
                else:
                    self.get_logger().error("Failed to identify egg.")
                    break

            elif self.state == State.HANDLING_EGG:
                asyncio.run(self.handle_egg(self.egg_type))
                self.state = State.COLLECTION_COMPLETE
            
            elif self.state == State.COLLECTION_COMPLETE:
                self.get_logger().info("Egg collection and sorting complete.")
                self.state = State.IDLE

        self.state = State.IDLE

    async def identify_egg(self):
        self.get_logger().info("Waiting for egg identification service...")
        await self.egg_id_client.wait_for_service()
        request = IdentifyEgg.Request()
        future = self.egg_id_client.call_async(request)
        await future
        response = future.result()
        if response:
            self.get_logger().info(f"Egg identified as type: {response.egg_type}")
            return response.egg_type
        return None

    async def handle_egg(self, egg_type):
        goal_msg = GenericTask.Goal()
        # NOTE: GenericTask does not support sending data. A custom action is recommended.
        # For now, we are not passing the egg_type to the handle_fsm.
        # The user should create a custom action `HandleEgg.action` that takes `egg_type` as a goal.
        self.get_logger().info(f"Requesting egg handling for type: {egg_type}")
        
        await self.handle_egg_client.wait_for_server()
        send_goal_future = self.handle_egg_client.send_goal_async(goal_msg)
        await send_goal_future
        goal_handle = send_goal_future.result()

        if not goal_handle.accepted:
            self.get_logger().error("Handle egg goal rejected")
            return
...
def main(args=None):
    rclpy.init(args=args)
    collect_fsm = CollectFSM()
    executor = MultiThreadedExecutor()
    executor.add_node(collect_fsm)
    executor.spin()
